...
test_that("continuous comparison works with bundled base-condition metadata", {
  ...
  de <- ProtPipe2::do_comparison_continuous(se, "day_num")

  expect_true(all(c("rho", "P.Value", "adj.P.Val") %in% names(de)))
  expect_s3_class(ProtPipe2::plot_correlation_volcano(de, label_col = "PG.Genes"), "ggplot")
})
...
test_that("custom ontology wrappers work on simple local gene sets", {
  ...
  ora <- ProtPipe2::enrich_terms(
    gene_id = as.character(1:10),
    all_gene_vector = universe,
    term2gene = term2gene,
    term2name = term2name
  )
  gsea <- ProtPipe2::gse_terms(
...
test_that("add_entrez maps gene symbols onto differential results", {
  ...
  mapped <- ProtPipe2::add_entrez(de, gene_col = "PG.Genes")
...
test_that("GO and KEGG wrappers handle invalid IDs gracefully", {
  ...
  expect_null(ProtPipe2::enrich_go(invalid_genes, invalid_genes))
  expect_null(ProtPipe2::gse_go(invalid_ranked))
  expect_null(ProtPipe2::enrich_kegg(invalid_genes, invalid_genes))
  expect_null(ProtPipe2::gse_kegg(invalid_ranked))
})
...
test_that("enrich_pathways short-circuits on comparisons with no signal", {
  ...
  enrichment <- ProtPipe2::enrich_pathways(no_signal, gene_col = "PG.Genes")
